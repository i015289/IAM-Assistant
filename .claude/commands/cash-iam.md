...
>
> **Authorization & Activity Validation**
> - "Review the authorization setup for IAM App ID `<IAM_APP_ID>` — are activity sets and authorization objects correctly maintained?"
> - "Validate whether approval activities such as 31 (Confirm) or 63 (Activate) are correctly restricted to approval applications only."
>
> **Missing Authorization Analysis**
> - "User is missing authorization object `<AUTH_OBJECT>` with activity `<ACTVT>` — identify the required catalog, role, or BRT."
> - "Analyze the authorization error for IAM App ID `<IAM_APP_ID>` and determine the missing access."
>
> **SoD & Four-Eyes Principle Validation**
> - "Analyze whether submit and approve capabilities are properly segregated for IAM App ID `<IAM_APP_ID>`."
> - "Identify whether any role, catalog, or BRT combination introduces self-approval or creator-approver risks."
>
> **SU22 & Transport Validation**
> - "Validate whether SU22 updates or transport imports introduced unexpected authorization changes for IAM App ID `<IAM_APP_ID>`."
> - "Compare authorization defaults before and after transport `<TRANSPORT_ID>` to identify inconsistencies or over-authorizations."
>
> **Catalog & BRT Structure Analysis**
> - "Analyze catalog dependencies and prerequisite relationships for Business Catalog `<BC_ID>`."
> - "Provide the complete catalog and application footprint for Business Role `<ROLE_ID>`."
>
> **Regular IAM Health Checks**
> - "Run a full IAM health check for IAM App ID `<IAM_APP_ID>`, including authorization objects, activity sets, catalogs, and BRT coverage."
> - "Validate whether read-only applications and approval applications are correctly designed and consistently implemented."
