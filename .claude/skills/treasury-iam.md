...
Use `APS_IAM_W_BC_APP` for business catalog lookups — `SUI_TM_MM_APP` stores Treasury app IDs as GUIDs and cannot be filtered by SIA6 name.
...
To list all apps in a known catalog:
```sql
SELECT BU_CATALOG_ID, APP_ID FROM APS_IAM_W_BC_APP
